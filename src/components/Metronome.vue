<template>
  ...
  <v-container fluid>
      <v-row>
          ...
          <v-col class="text-center" v-for="note in subdivisionArr" :key="note.id" @click="activeMusicNote(note)">
            <v-icon :color="note.isActived?'red':'black'">{{note.iconName}}</v-icon>
          </v-col>
      </v-row>
  </v-container>
...
export default {
  ...
  data:() => ({
      ...
      circleObj:[{ //Circles in the user interface
        id:0,
        isActive:false
      },{
        id:1,
  ...
  watch:{
    ...
    upperNumeral:function(newUpperNumeral){
       ...
       */
        ...
        const note = musicNotes.filter((note)=>note.correspondingNum === noteCorrespondingNum)[0]
  ...
  methods:{
      ...
      countBeat:function(){
        ...
        if(upperNumeral === 1) return this.circleObj[this.currentBeat].isActive = true //if there is only 1 beat in a bar, we need to control only 1 circle in interface 
